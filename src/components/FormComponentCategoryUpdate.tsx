...
export default function FormComponentCategoryUpdate({ id }: { id: number }) {
  ...
  async function handleUpdate(e: FormEvent<HTMLButtonElement>) {
    ...
    if (data && data.name.trim()) {
      try {
        const response: unknown = await patchCategory({
          data: {
            id,
            name: data.name,
          },
        ...
        if (categoryResponse && categoryResponse.status === 200) {
          ...
          dispatch(
            setError("Erro:  ocorreu um erro durante ao edição da categoria")
          );
        }
      } catch (error) {
        console.log(error);
        dispatch(
          setError(
            "Erro: Ocorreu um erro durante o processo de edição da categoria."
          )
        );
      }
    } else {
        ...
        <>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                dispatch(setCategory({ id: id, name: e.target.value }))
              }
              required
              sx={{
                width: {
